...
async function loadInvestigationMeta() {
  ...
  state.originReference = meta.originReference || "";
}

const VERSE_REFERENCE = /\s\d+:\d+$/u;

function isVerseReference(value) {
  return VERSE_REFERENCE.test(String(value || "").trim());
}

function bookFromReference(value) {
  return String(value || "").trim().replace(/\s+\d+:\d+$/u, "").trim();
}

/**
 * Scope Reference means different things per scope, so it cannot have one default:
 *   Occurrence   - matched by exact equality against a verse reference, so it must be
 *                  "<Book> <chapter>:<verse>". A bare book name never matches and would
 *                  leave the decision approved but permanently inert.
 *   Book Default - never read by the matcher; the book name is the useful label.
 *   Construction - not used at all; the condition carries the scope.
 *
 * Runs only on an explicit scope change by the user, never while loading a record, so
 * opening an investigation can never silently rewrite what is stored on disk.
 */
function applyScopeDefaults() {
  const scope = decisionScope.value;
  const origin = String(state.originReference || "").trim();
  const current = decisionScopeReference.value.trim();

  if (scope === "Occurrence") {
    if (!isVerseReference(current)) decisionScopeReference.value = origin || current;
  } else if (scope === "Book Default") {
    if (!current || isVerseReference(current)) decisionScopeReference.value = bookFromReference(origin) || current;
  } else if (scope === "Construction") {
    decisionScopeReference.value = "";
  }
  updateScopeFieldAvailability();
}

function updateScopeFieldAvailability() {
  const scope = decisionScope.value;
  decisionScopeReference.disabled = scope === "Construction";
  decisionScopeCondition.disabled = scope !== "Construction";
}
...
decisionScope.addEventListener("change", () => {
  applyScopeDefaults();
  state.dirty = true;
  setStatus("Unsaved changes", "dirty");
});
...
approveDecision.addEventListener("click", () => {
  void (async () => {
    ...
    if (scope === "Occurrence" && !isVerseReference(decisionScopeReference.value)) {
      prototypeMessage.textContent =
        "Occurrence scope needs a chapter:verse reference (e.g. Titus 1:1). "
        + "A bare book name never matches a verse, so the decision would never apply.";
      decisionScopeReference.focus();
      return;
    }
