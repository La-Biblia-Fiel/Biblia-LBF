...
/**
 * Occurrence scope is matched by exact string equality against a verse reference
 * (see policyApplicability). A bare book name therefore never matches any verse and
 * silently disables the decision, so the format is required, not merely non-empty.
 */
export function isVerseReference(value = "") {
  return VERSE_REFERENCE.test(String(value || "").trim());
}

/** "Daniel 10:6" -> "Daniel"; "1 Timoteo 2:5" -> "1 Timoteo"; "Titus" -> "Titus". */
export function bookFromReference(value = "") {
...
export function validateDecisionScope({ scope = "", scopeReference = "", scopeCondition = "" } = {}) {
  ...
  if (normalizedScope === "Occurrence" && !isVerseReference(scopeReference)) {
    return {
      valid: false,
      error: "Occurrence scope requires a chapter:verse reference (e.g. Titus 1:1), not a book name."
    };
  }
