...
def base_lemma(value: str) -> str:
    """Identity of an OSHB lemma is its final component.

    OSHB writes prefixed forms as compound lemmas ("c/5174", "c/l/2608 a"), so exact
    string equality would treat a prefixed occurrence as a different word and silently
    exclude it from a book-wide rendering change.
    """
    return str(value or "").strip().split("/")[-1].strip()


def token_id(token: dict) -> tuple[str, str]:
    return str(token.get("strongs") or "").strip().upper(), base_lemma(token.get("lemma"))
