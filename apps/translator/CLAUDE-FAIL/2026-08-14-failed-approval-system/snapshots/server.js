...
import { isVerseReference } from "./src/investigations/decisionScope.js";
...
async function handleDecision(request, response, id) {
  ...
  if (body.action === "approve") {
    ...
    if (current.scope === "Occurrence" && !isVerseReference(current.scopeReference)) {
      sendJson(response, 400, {
        error: "Occurrence scope requires a chapter:verse Scope Reference (e.g. Titus 1:1), not a book name. "
          + "A bare book name never matches a verse, so the decision would be approved but never apply."
      });
      return;
    }
