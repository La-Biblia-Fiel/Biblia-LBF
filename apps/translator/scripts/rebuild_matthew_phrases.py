#!/usr/bin/env python3
"""Rebuild Matthew (Mateo) phrase map + phrases.json from filled LBF span seed."""
...
HERR = ROOT.parent
MORPH = HERR / "MNA" / "SOURCES" / "MorphGNT" / "61-Mt-morphgnt.txt"
BLE = HERR / "Biblia-BLE" / "output" / "mateo.ble.md"
FILLED = ROOT / "translations" / "phrase-maps" / "matthew-spans-filled.json"
OUT_MAP = ROOT / "translations" / "phrase-maps" / "matthew.json"
OUT_PHRASES = ROOT / "translations" / "matthew-phrases.json"
OUT_DOC = ROOT / "translations" / "mateo.md"
LBF_OUT = HERR / "Biblia-LBF" / "translation" / "nt" / "mateo.md"
...
def load_ble() -> dict[tuple[int, int], str]:
    ...
    for line in BLE.read_text(encoding="utf-8").splitlines():
        m = re.match(r"^(?:mateo|Mateo)\s+(\d+):(\d+)\s+(.+)$", line)
        if m:
...
def main() -> int:
    ...
    OUT_MAP.write_text(
        json.dumps({"book": "matthew", "version": 1, "phrases": phrase_map}, ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
    )
