#!/usr/bin/env python3
"""Rebuild 2 Timothy (2 Timoteo) phrase map + phrases.json from filled LBF span seed."""
...
HERR = ROOT.parent
MORPH = HERR / "MNA" / "SOURCES" / "MorphGNT" / "76-2Ti-morphgnt.txt"
BLE = HERR / "Biblia-BLE" / "output" / "2timoteo.ble.md"
FILLED = ROOT / "translations" / "phrase-maps" / "2timothy-spans-filled.json"
OUT_MAP = ROOT / "translations" / "phrase-maps" / "2timothy.json"
OUT_PHRASES = ROOT / "translations" / "2timothy-phrases.json"
OUT_DOC = ROOT / "translations" / "2timoteo.md"
LBF_OUT = HERR / "Biblia-LBF" / "translation" / "nt" / "2timoteo.md"
STRONGS = HERR / "MNA" / "datasets" / "rules" / "grc_lemma_strongs.json"

BOOK_CODE = 55
BOOK_LABEL = "2 Timothy"
DOC_TITLE = "2 Timoteo"
...
def load_ble() -> dict[tuple[int, int], str]:
    ...
    for line in BLE.read_text(encoding="utf-8").splitlines():
        m = re.match(r"^(?:2timoteo|2Timoteo|2\s*Timoteo)\s+(\d+):(\d+)\s+(.+)$", line)
        if m:
...
def main() -> int:
    ...
    OUT_MAP.write_text(
        json.dumps({"book": "2timothy", "version": 1, "phrases": phrase_map}, ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
    )
