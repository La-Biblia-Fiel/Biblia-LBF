#!/usr/bin/env python3
"""Rebuild Revelation (Apocalipsis) phrase map + phrases.json from filled LBF span seed."""
...
BLE = HERR / "Biblia-BLE" / "output" / "apocalipsis.ble.md"
FILLED = ROOT / "translations" / "phrase-maps" / "revelation-spans-filled.json"
OUT_MAP = ROOT / "translations" / "phrase-maps" / "revelation.json"
OUT_PHRASES = ROOT / "translations" / "revelation-phrases.json"
OUT_DOC = ROOT / "translations" / "apocalipsis.md"
LBF_OUT = HERR / "Biblia-LBF" / "translation" / "nt" / "apocalipsis.md"
STRONGS = HERR / "MNA" / "datasets" / "rules" / "grc_lemma_strongs.json"

BOOK_CODE = 66
BOOK_LABEL = "Revelation"
DOC_TITLE = "Apocalipsis"
...
def load_ble() -> dict[tuple[int, int], str]:
    ...
    for line in BLE.read_text(encoding="utf-8").splitlines():
        m = re.match(r"^(?:apocalipsis|Apocalipsis)\s+(\d+):(\d+)\s+(.+)$", line)
        if m:
...
def main() -> int:
    ...
    OUT_MAP.write_text(
        json.dumps({"book": "revelation", "version": 1, "phrases": phrase_map}, ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
    )
