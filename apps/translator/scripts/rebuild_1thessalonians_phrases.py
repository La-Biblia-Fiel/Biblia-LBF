#!/usr/bin/env python3
"""Rebuild 1 Thessalonians (1 Tesalonicenses) phrase map + phrases.json from filled LBF span seed."""
...
HERR = ROOT.parent
MORPH = HERR / "MNA" / "SOURCES" / "MorphGNT" / "73-1Th-morphgnt.txt"
BLE = HERR / "Biblia-BLE" / "output" / "1tesalonicenses.ble.md"
FILLED = ROOT / "translations" / "phrase-maps" / "1thessalonians-spans-filled.json"
OUT_MAP = ROOT / "translations" / "phrase-maps" / "1thessalonians.json"
OUT_PHRASES = ROOT / "translations" / "1thessalonians-phrases.json"
OUT_DOC = ROOT / "translations" / "1tesalonicenses.md"
LBF_OUT = HERR / "Biblia-LBF" / "translation" / "nt" / "1tesalonicenses.md"
STRONGS = HERR / "MNA" / "datasets" / "rules" / "grc_lemma_strongs.json"

BOOK_CODE = 52
BOOK_LABEL = "1 Thessalonians"
DOC_TITLE = "1 Tesalonicenses"
...
def load_ble() -> dict[tuple[int, int], str]:
    ...
    for line in BLE.read_text(encoding="utf-8").splitlines():
        m = re.match(r"^(?:1tesalonicenses|1Tesalonicenses|1\s*Tesalonicenses)\s+(\d+):(\d+)\s+(.+)$", line)
        if m:
...
def main() -> int:
    ...
    OUT_MAP.write_text(
        json.dumps({"book": "1thessalonians", "version": 1, "phrases": phrase_map}, ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
    )
