#!/usr/bin/env python3
"""Rebuild 2 John (2 Juan) phrase map + phrases.json from filled LBF span seed."""
...
HERR = ROOT.parent
MORPH = HERR / "MNA" / "SOURCES" / "MorphGNT" / "84-2Jn-morphgnt.txt"
BLE = HERR / "Biblia-BLE" / "output" / "2juan.ble.md"
FILLED = ROOT / "translations" / "phrase-maps" / "2john-spans-filled.json"
OUT_MAP = ROOT / "translations" / "phrase-maps" / "2john.json"
OUT_PHRASES = ROOT / "translations" / "2john-phrases.json"
OUT_DOC = ROOT / "translations" / "2juan.md"
LBF_OUT = HERR / "Biblia-LBF" / "translation" / "nt" / "2juan.md"
...
def load_ble() -> dict[tuple[int, int], str]:
    ...
    for line in BLE.read_text(encoding="utf-8").splitlines():
        m = re.match(r"^(?:2juan|1Juan|1\s*Juan)\s+(\d+):(\d+)\s+(.+)$", line)
        if m:
...
def main() -> int:
    ...
    OUT_MAP.write_text(
        json.dumps({"book": "2john", "version": 1, "phrases": phrase_map}, ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
    )
