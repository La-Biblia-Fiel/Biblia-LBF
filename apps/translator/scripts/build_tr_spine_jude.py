#!/usr/bin/env python3
"""Build Jude TR spine from Robinson-parsed Scrivener (UTR) + remap phrases.

Primary textual/morph source:
  Biblia-LBF/source/greek/TR1894/robinson-parsed/JUDE.UTR
...
UTR_PATH = HERR / "Biblia-LBF/source/greek/TR1894/robinson-parsed/JUDE.UTR"
SCV_PATH = HERR / "Biblia-LBF/source/greek/TR1894/scrivener-textonly/JUDE.SCV"
TR_ACCENT_PATH = HERR / "Biblia-LBF/source/greek/TR1894/tr1894.txt"
MORPH = HERR / "MNA/SOURCES/MorphGNT/86-Jud-morphgnt.txt"
PHRASES = HERR / "cgv-translator/translations/jude-phrases.json"
OUT_DIR = HERR / "cgv-translator/translations/tr-spine/jude"
BOOK_CODE = 65
...
def main() -> None:
    ...
    (OUT_DIR / "jude-tr-spine.json").write_text(
    ...
    (OUT_DIR / "jude-phrases-tr.json").write_text(
    ...
    (OUT_DIR / "jude-tr-diff-report.md").write_text(
        "\n".join(lines).rstrip() + "\n", encoding="utf-8"
    )
    (OUT_DIR / "jude-phrase-remap-issues.json").write_text(
