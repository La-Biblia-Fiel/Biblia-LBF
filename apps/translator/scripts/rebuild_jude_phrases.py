#!/usr/bin/env python3
"""Rebuild Jude (Judas) phrase map + phrases.json from filled LBF span seed."""
...
BLE = HERR / "Biblia-BLE" / "output" / "judas.ble.md"
FILLED = ROOT / "translations" / "phrase-maps" / "jude-spans-filled.json"
OUT_MAP = ROOT / "translations" / "phrase-maps" / "jude.json"
OUT_PHRASES = ROOT / "translations" / "jude-phrases.json"
OUT_DOC = ROOT / "translations" / "judas.md"
LBF_OUT = HERR / "Biblia-LBF" / "translation" / "nt" / "judas.md"
...
def load_ble() -> dict[tuple[int, int], str]:
    ...
    for line in BLE.read_text(encoding="utf-8").splitlines():
        m = re.match(r"^(?:judas|1Juan|1\s*Juan)\s+(\d+):(\d+)\s+(.+)$", line)
        if m:
...
def main() -> int:
    ...
    OUT_MAP.write_text(
        json.dumps({"book": "jude", "version": 1, "phrases": phrase_map}, ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
    )
