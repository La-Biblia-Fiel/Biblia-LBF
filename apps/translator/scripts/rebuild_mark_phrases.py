#!/usr/bin/env python3
"""Rebuild Mark (Marcos) phrase map + phrases.json from filled LBF span seed."""
...
HERR = ROOT.parent
MORPH = HERR / "MNA" / "SOURCES" / "MorphGNT" / "62-Mk-morphgnt.txt"
BLE = HERR / "Biblia-BLE" / "output" / "marcos.ble.md"
FILLED = ROOT / "translations" / "phrase-maps" / "mark-spans-filled.json"
OUT_MAP = ROOT / "translations" / "phrase-maps" / "mark.json"
OUT_PHRASES = ROOT / "translations" / "mark-phrases.json"
OUT_DOC = ROOT / "translations" / "marcos.md"
LBF_OUT = HERR / "Biblia-LBF" / "translation" / "nt" / "marcos.md"
...
def load_ble() -> dict[tuple[int, int], str]:
    ...
    for line in BLE.read_text(encoding="utf-8").splitlines():
        m = re.match(r"^(?:marcos|Marcos)\s+(\d+):(\d+)\s+(.+)$", line)
        if m:
...
def main() -> int:
    ...
    OUT_MAP.write_text(
        json.dumps({"book": "mark", "version": 1, "phrases": phrase_map}, ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
    )
