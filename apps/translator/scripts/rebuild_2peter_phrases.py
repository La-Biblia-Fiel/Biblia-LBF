#!/usr/bin/env python3
"""Rebuild 2 Peter (2 Pedro) phrase map + phrases.json from filled LBF span seed."""
...
HERR = ROOT.parent
MORPH = HERR / "MNA" / "SOURCES" / "MorphGNT" / "82-2Pe-morphgnt.txt"
BLE = HERR / "Biblia-BLE" / "output" / "2pedro.ble.md"
FILLED = ROOT / "translations" / "phrase-maps" / "2peter-spans-filled.json"
OUT_MAP = ROOT / "translations" / "phrase-maps" / "2peter.json"
OUT_PHRASES = ROOT / "translations" / "2peter-phrases.json"
OUT_DOC = ROOT / "translations" / "2pedro.md"
LBF_OUT = HERR / "Biblia-LBF" / "translation" / "nt" / "2pedro.md"
...
def load_ble() -> dict[tuple[int, int], str]:
    ...
    for line in BLE.read_text(encoding="utf-8").splitlines():
        m = re.match(r"^(?:2pedro|2Pedro|2\s*Pedro)\s+(\d+):(\d+)\s+(.+)$", line)
        if m:
...
def main() -> int:
    ...
    OUT_MAP.write_text(
        json.dumps({"book": "2peter", "version": 1, "phrases": phrase_map}, ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
    )
