#!/usr/bin/env python3
"""Rebuild 2 Thessalonians (2 Tesalonicenses) phrase map + phrases.json from filled LBF span seed."""
...
HERR = ROOT.parent
MORPH = HERR / "MNA" / "SOURCES" / "MorphGNT" / "74-2Th-morphgnt.txt"
BLE = HERR / "Biblia-BLE" / "output" / "2tesalonicenses.ble.md"
FILLED = ROOT / "translations" / "phrase-maps" / "2thessalonians-spans-filled.json"
OUT_MAP = ROOT / "translations" / "phrase-maps" / "2thessalonians.json"
OUT_PHRASES = ROOT / "translations" / "2thessalonians-phrases.json"
OUT_DOC = ROOT / "translations" / "2tesalonicenses.md"
LBF_OUT = HERR / "Biblia-LBF" / "translation" / "nt" / "2tesalonicenses.md"
STRONGS = HERR / "MNA" / "datasets" / "rules" / "grc_lemma_strongs.json"

BOOK_CODE = 53
BOOK_LABEL = "2 Thessalonians"
DOC_TITLE = "2 Tesalonicenses"
...
def load_ble() -> dict[tuple[int, int], str]:
    ...
    for line in BLE.read_text(encoding="utf-8").splitlines():
        m = re.match(r"^(?:2tesalonicenses|2Tesalonicenses|2\s*Tesalonicenses)\s+(\d+):(\d+)\s+(.+)$", line)
        if m:
...
def main() -> int:
    ...
    OUT_MAP.write_text(
        json.dumps({"book": "2thessalonians", "version": 1, "phrases": phrase_map}, ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
    )
