#!/usr/bin/env python3
"""Rebuild Philippians (Filipenses) phrase map + phrases.json from filled LBF span seed."""
...
HERR = ROOT.parent
MORPH = HERR / "MNA" / "SOURCES" / "MorphGNT" / "71-Php-morphgnt.txt"
BLE = HERR / "Biblia-BLE" / "output" / "filipenses.ble.md"
FILLED = ROOT / "translations" / "phrase-maps" / "philippians-spans-filled.json"
OUT_MAP = ROOT / "translations" / "phrase-maps" / "philippians.json"
OUT_PHRASES = ROOT / "translations" / "philippians-phrases.json"
OUT_DOC = ROOT / "translations" / "filipenses.md"
LBF_OUT = HERR / "Biblia-LBF" / "translation" / "nt" / "filipenses.md"
STRONGS = HERR / "MNA" / "datasets" / "rules" / "grc_lemma_strongs.json"

BOOK_CODE = 50
BOOK_LABEL = "Philippians"
DOC_TITLE = "Filipenses"
...
def load_ble() -> dict[tuple[int, int], str]:
    ...
    for line in BLE.read_text(encoding="utf-8").splitlines():
        m = re.match(r"^(?:filipenses|Filipenses)\s+(\d+):(\d+)\s+(.+)$", line)
        if m:
...
def main() -> int:
    ...
    OUT_MAP.write_text(
        json.dumps({"book": "philippians", "version": 1, "phrases": phrase_map}, ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
    )
