#!/usr/bin/env python3
"""Rebuild Galatians (Gálatas) phrase map + phrases.json from filled LBF span seed."""
...
HERR = ROOT.parent
MORPH = HERR / "MNA" / "SOURCES" / "MorphGNT" / "69-Ga-morphgnt.txt"
BLE = HERR / "Biblia-BLE" / "output" / "galatas.ble.md"
FILLED = ROOT / "translations" / "phrase-maps" / "galatians-spans-filled.json"
OUT_MAP = ROOT / "translations" / "phrase-maps" / "galatians.json"
OUT_PHRASES = ROOT / "translations" / "galatians-phrases.json"
OUT_DOC = ROOT / "translations" / "galatas.md"
LBF_OUT = HERR / "Biblia-LBF" / "translation" / "nt" / "galatas.md"
STRONGS = HERR / "MNA" / "datasets" / "rules" / "grc_lemma_strongs.json"

BOOK_CODE = 48
BOOK_LABEL = "Galatians"
DOC_TITLE = "Gálatas"
...
def load_ble() -> dict[tuple[int, int], str]:
    ...
    for line in BLE.read_text(encoding="utf-8").splitlines():
        m = re.match(r"^(?:galatas|2Corintios|2\s*Corintios)\s+(\d+):(\d+)\s+(.+)$", line)
        if m:
...
def main() -> int:
    ...
    OUT_MAP.write_text(
        json.dumps({"book": "galatians", "version": 1, "phrases": phrase_map}, ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
    )
