#!/usr/bin/env python3
"""Rebuild Ephesians (Efesios) phrase map + phrases.json from filled LBF span seed."""
...
HERR = ROOT.parent
MORPH = HERR / "MNA" / "SOURCES" / "MorphGNT" / "70-Eph-morphgnt.txt"
BLE = HERR / "Biblia-BLE" / "output" / "efesios.ble.md"
FILLED = ROOT / "translations" / "phrase-maps" / "ephesians-spans-filled.json"
OUT_MAP = ROOT / "translations" / "phrase-maps" / "ephesians.json"
OUT_PHRASES = ROOT / "translations" / "ephesians-phrases.json"
OUT_DOC = ROOT / "translations" / "efesios.md"
LBF_OUT = HERR / "Biblia-LBF" / "translation" / "nt" / "efesios.md"
STRONGS = HERR / "MNA" / "datasets" / "rules" / "grc_lemma_strongs.json"

BOOK_CODE = 49
BOOK_LABEL = "Ephesians"
DOC_TITLE = "Efesios"
...
def load_ble() -> dict[tuple[int, int], str]:
    ...
    for line in BLE.read_text(encoding="utf-8").splitlines():
        m = re.match(r"^(?:efesios|Efesios)\s+(\d+):(\d+)\s+(.+)$", line)
        if m:
...
def main() -> int:
    ...
    OUT_MAP.write_text(
        json.dumps({"book": "ephesians", "version": 1, "phrases": phrase_map}, ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
    )
