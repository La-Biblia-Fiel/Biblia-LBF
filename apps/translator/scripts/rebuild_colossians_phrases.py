#!/usr/bin/env python3
"""Rebuild Colossians (Colosenses) phrase map + phrases.json from filled LBF span seed."""
...
HERR = ROOT.parent
MORPH = HERR / "MNA" / "SOURCES" / "MorphGNT" / "72-Col-morphgnt.txt"
BLE = HERR / "Biblia-BLE" / "output" / "colosenses.ble.md"
FILLED = ROOT / "translations" / "phrase-maps" / "colossians-spans-filled.json"
OUT_MAP = ROOT / "translations" / "phrase-maps" / "colossians.json"
OUT_PHRASES = ROOT / "translations" / "colossians-phrases.json"
OUT_DOC = ROOT / "translations" / "colosenses.md"
LBF_OUT = HERR / "Biblia-LBF" / "translation" / "nt" / "colosenses.md"
STRONGS = HERR / "MNA" / "datasets" / "rules" / "grc_lemma_strongs.json"

BOOK_CODE = 51
BOOK_LABEL = "Colossians"
DOC_TITLE = "Colosenses"
...
def load_ble() -> dict[tuple[int, int], str]:
    ...
    for line in BLE.read_text(encoding="utf-8").splitlines():
        m = re.match(r"^(?:colosenses|Colosenses)\s+(\d+):(\d+)\s+(.+)$", line)
        if m:
...
def main() -> int:
    ...
    OUT_MAP.write_text(
        json.dumps({"book": "colossians", "version": 1, "phrases": phrase_map}, ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
    )
