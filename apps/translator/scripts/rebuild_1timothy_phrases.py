#!/usr/bin/env python3
"""Rebuild 1 Timothy (1 Timoteo) phrase map + phrases.json from filled LBF span seed."""
...
HERR = ROOT.parent
MORPH = HERR / "MNA" / "SOURCES" / "MorphGNT" / "75-1Ti-morphgnt.txt"
BLE = HERR / "Biblia-BLE" / "output" / "1timoteo.ble.md"
FILLED = ROOT / "translations" / "phrase-maps" / "1timothy-spans-filled.json"
OUT_MAP = ROOT / "translations" / "phrase-maps" / "1timothy.json"
OUT_PHRASES = ROOT / "translations" / "1timothy-phrases.json"
OUT_DOC = ROOT / "translations" / "1timoteo.md"
LBF_OUT = HERR / "Biblia-LBF" / "translation" / "nt" / "1timoteo.md"
STRONGS = HERR / "MNA" / "datasets" / "rules" / "grc_lemma_strongs.json"

BOOK_CODE = 54
BOOK_LABEL = "1 Timothy"
DOC_TITLE = "1 Timoteo"
...
def load_ble() -> dict[tuple[int, int], str]:
    ...
    for line in BLE.read_text(encoding="utf-8").splitlines():
        m = re.match(r"^(?:1timoteo|1Timoteo|1\s*Timoteo)\s+(\d+):(\d+)\s+(.+)$", line)
        if m:
...
def main() -> int:
    ...
    OUT_MAP.write_text(
        json.dumps({"book": "1timothy", "version": 1, "phrases": phrase_map}, ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
    )
