#!/usr/bin/env python3
"""Rebuild 1 Corinthians (1 Corintios) phrase map + phrases.json from filled LBF span seed."""
...
HERR = ROOT.parent
MORPH = HERR / "MNA" / "SOURCES" / "MorphGNT" / "67-1Co-morphgnt.txt"
BLE = HERR / "Biblia-BLE" / "output" / "1corintios.ble.md"
FILLED = ROOT / "translations" / "phrase-maps" / "1corinthians-spans-filled.json"
OUT_MAP = ROOT / "translations" / "phrase-maps" / "1corinthians.json"
OUT_PHRASES = ROOT / "translations" / "1corinthians-phrases.json"
OUT_DOC = ROOT / "translations" / "1corintios.md"
LBF_OUT = HERR / "Biblia-LBF" / "translation" / "nt" / "1corintios.md"
STRONGS = HERR / "MNA" / "datasets" / "rules" / "grc_lemma_strongs.json"

BOOK_CODE = 46
BOOK_LABEL = "1 Corinthians"
DOC_TITLE = "1 Corintios"
...
def load_ble() -> dict[tuple[int, int], str]:
    ...
    for line in BLE.read_text(encoding="utf-8").splitlines():
        m = re.match(r"^(?:1corintios|1Corintios|1\s*Corintios)\s+(\d+):(\d+)\s+(.+)$", line)
        if m:
...
def main() -> int:
    ...
    OUT_MAP.write_text(
        json.dumps({"book": "1corinthians", "version": 1, "phrases": phrase_map}, ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
    )
