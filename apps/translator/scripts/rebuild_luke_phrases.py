#!/usr/bin/env python3
"""Rebuild Luke (Lucas) phrase map + phrases.json from filled LBF span seed."""
...
HERR = ROOT.parent
MORPH = HERR / "MNA" / "SOURCES" / "MorphGNT" / "63-Lk-morphgnt.txt"
BLE = HERR / "Biblia-BLE" / "output" / "lucas.ble.md"
FILLED = ROOT / "translations" / "phrase-maps" / "luke-spans-filled.json"
OUT_MAP = ROOT / "translations" / "phrase-maps" / "luke.json"
OUT_PHRASES = ROOT / "translations" / "luke-phrases.json"
OUT_DOC = ROOT / "translations" / "lucas.md"
LBF_OUT = HERR / "Biblia-LBF" / "translation" / "nt" / "lucas.md"
...
def load_ble() -> dict[tuple[int, int], str]:
    ...
    for line in BLE.read_text(encoding="utf-8").splitlines():
        m = re.match(r"^(?:lucas|Lucas)\s+(\d+):(\d+)\s+(.+)$", line)
        if m:
...
def main() -> int:
    ...
    OUT_MAP.write_text(
        json.dumps({"book": "luke", "version": 1, "phrases": phrase_map}, ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
    )
