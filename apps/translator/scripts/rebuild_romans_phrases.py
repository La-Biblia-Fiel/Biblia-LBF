#!/usr/bin/env python3
"""Rebuild Romans (Romanos) phrase map + phrases.json from filled LBF span seed."""
...
HERR = ROOT.parent
MORPH = HERR / "MNA" / "SOURCES" / "MorphGNT" / "66-Ro-morphgnt.txt"
BLE = HERR / "Biblia-BLE" / "output" / "romanos.ble.md"
FILLED = ROOT / "translations" / "phrase-maps" / "romans-spans-filled.json"
OUT_MAP = ROOT / "translations" / "phrase-maps" / "romans.json"
OUT_PHRASES = ROOT / "translations" / "romans-phrases.json"
OUT_DOC = ROOT / "translations" / "romanos.md"
LBF_OUT = HERR / "Biblia-LBF" / "translation" / "nt" / "romanos.md"
...
def load_ble() -> dict[tuple[int, int], str]:
    ...
    for line in BLE.read_text(encoding="utf-8").splitlines():
        m = re.match(r"^(?:romanos|Romanos)\s+(\d+):(\d+)\s+(.+)$", line)
        if m:
...
def main() -> int:
    ...
    OUT_MAP.write_text(
        json.dumps({"book": "romans", "version": 1, "phrases": phrase_map}, ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
    )
