#!/usr/bin/env python3
"""Rebuild Hebrews phrase map + phrases.json from filled LBF span seed."""
...
HERR = ROOT.parent
MORPH = HERR / "MNA" / "SOURCES" / "MorphGNT" / "79-Heb-morphgnt.txt"
BLE = HERR / "Biblia-BLE" / "output" / "hebreos.ble.md"
FILLED = ROOT / "translations" / "phrase-maps" / "hebrews-spans-filled.json"
OUT_MAP = ROOT / "translations" / "phrase-maps" / "hebrews.json"
OUT_PHRASES = ROOT / "translations" / "hebrews-phrases.json"
OUT_DOC = ROOT / "translations" / "hebreos.md"
LBF_OUT = HERR / "Biblia-LBF" / "translation" / "nt" / "hebreos.md"
...
def load_ble() -> dict[tuple[int, int], str]:
    ...
    for line in BLE.read_text(encoding="utf-8").splitlines():
        m = re.match(r"^Hebreos\s+(\d+):(\d+)\s+(.+)$", line)
        if m:
...
def main() -> int:
    ...
    # fix localIndex per verse
    local_counts: dict[tuple[int, int], int] = defaultdict(int)
    for item in phrase_map:
        m = re.search(r"(\d+):(\d+)$", item["reference"])
        key = (int(m.group(1)), int(m.group(2)))
        item["localIndex"] = local_counts[key]
        local_counts[key] += 1

    OUT_MAP.parent.mkdir(parents=True, exist_ok=True)
    OUT_MAP.write_text(
        json.dumps({"book": "hebrews", "version": 1, "phrases": phrase_map}, ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
    )
