#!/usr/bin/env python3
"""Rebuild John (Juan) phrase map + phrases.json from filled LBF span seed."""
...
HERR = ROOT.parent
MORPH = HERR / "MNA" / "SOURCES" / "MorphGNT" / "64-Jn-morphgnt.txt"
BLE = HERR / "Biblia-BLE" / "output" / "juan.ble.md"
FILLED = ROOT / "translations" / "phrase-maps" / "john-spans-filled.json"
OUT_MAP = ROOT / "translations" / "phrase-maps" / "john.json"
OUT_PHRASES = ROOT / "translations" / "john-phrases.json"
OUT_DOC = ROOT / "translations" / "juan.md"
LBF_OUT = HERR / "Biblia-LBF" / "translation" / "nt" / "juan.md"
...
def load_ble() -> dict[tuple[int, int], str]:
    ...
    for line in BLE.read_text(encoding="utf-8").splitlines():
        m = re.match(r"^(?:juan|Juan)\s+(\d+):(\d+)\s+(.+)$", line)
        if m:
...
def main() -> int:
    ...
    OUT_MAP.write_text(
        json.dumps({"book": "john", "version": 1, "phrases": phrase_map}, ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
    )
