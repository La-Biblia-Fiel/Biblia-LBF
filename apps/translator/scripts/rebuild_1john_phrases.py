#!/usr/bin/env python3
"""Rebuild 1 John (1 Juan) phrase map + phrases.json from filled LBF span seed."""
...
BLE = HERR / "Biblia-BLE" / "output" / "1juan.ble.md"
FILLED = ROOT / "translations" / "phrase-maps" / "1john-spans-filled.json"
OUT_MAP = ROOT / "translations" / "phrase-maps" / "1john.json"
OUT_PHRASES = ROOT / "translations" / "1john-phrases.json"
OUT_DOC = ROOT / "translations" / "1juan.md"
LBF_OUT = HERR / "Biblia-LBF" / "translation" / "nt" / "1juan.md"
...
def load_ble() -> dict[tuple[int, int], str]:
    ...
    for line in BLE.read_text(encoding="utf-8").splitlines():
        m = re.match(r"^(?:1juan|1Juan|1\s*Juan)\s+(\d+):(\d+)\s+(.+)$", line)
        if m:
...
def main() -> int:
    ...
    OUT_MAP.write_text(
        json.dumps({"book": "1john", "version": 1, "phrases": phrase_map}, ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
    )
