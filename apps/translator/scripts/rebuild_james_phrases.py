#!/usr/bin/env python3
"""Rebuild James (Santiago) phrase map + phrases.json from filled LBF span seed."""
...
HERR = ROOT.parent
MORPH = HERR / "MNA" / "SOURCES" / "MorphGNT" / "80-Jas-morphgnt.txt"
BLE = HERR / "Biblia-BLE" / "output" / "santiago.ble.md"
FILLED = ROOT / "translations" / "phrase-maps" / "james-spans-filled.json"
OUT_MAP = ROOT / "translations" / "phrase-maps" / "james.json"
OUT_PHRASES = ROOT / "translations" / "james-phrases.json"
OUT_DOC = ROOT / "translations" / "santiago.md"
LBF_OUT = HERR / "Biblia-LBF" / "translation" / "nt" / "santiago.md"
...
def load_ble() -> dict[tuple[int, int], str]:
    ...
    for line in BLE.read_text(encoding="utf-8").splitlines():
        m = re.match(r"^Santiago\s+(\d+):(\d+)\s+(.+)$", line)
        if m:
...
def main() -> int:
    ...
    OUT_MAP.write_text(
        json.dumps({"book": "james", "version": 1, "phrases": phrase_map}, ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
    )
