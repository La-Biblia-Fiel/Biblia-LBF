#!/usr/bin/env python3
"""Rebuild Acts (Hechos) phrase map + phrases.json from filled LBF span seed."""
...
HERR = ROOT.parent
MORPH = HERR / "MNA" / "SOURCES" / "MorphGNT" / "65-Ac-morphgnt.txt"
BLE = HERR / "Biblia-BLE" / "output" / "hechos.ble.md"
FILLED = ROOT / "translations" / "phrase-maps" / "acts-spans-filled.json"
OUT_MAP = ROOT / "translations" / "phrase-maps" / "acts.json"
OUT_PHRASES = ROOT / "translations" / "acts-phrases.json"
OUT_DOC = ROOT / "translations" / "hechos.md"
LBF_OUT = HERR / "Biblia-LBF" / "translation" / "nt" / "hechos.md"
...
def load_ble() -> dict[tuple[int, int], str]:
    ...
    for line in BLE.read_text(encoding="utf-8").splitlines():
        m = re.match(r"^(?:hechos|Hechos)\s+(\d+):(\d+)\s+(.+)$", line)
        if m:
...
def main() -> int:
    ...
    OUT_MAP.write_text(
        json.dumps({"book": "acts", "version": 1, "phrases": phrase_map}, ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
    )
