#!/usr/bin/env python3
"""Rebuild 2 Corinthians (2 Corintios) phrase map + phrases.json from filled LBF span seed."""
...
HERR = ROOT.parent
MORPH = HERR / "MNA" / "SOURCES" / "MorphGNT" / "68-2Co-morphgnt.txt"
BLE = HERR / "Biblia-BLE" / "output" / "2corintios.ble.md"
FILLED = ROOT / "translations" / "phrase-maps" / "2corinthians-spans-filled.json"
OUT_MAP = ROOT / "translations" / "phrase-maps" / "2corinthians.json"
OUT_PHRASES = ROOT / "translations" / "2corinthians-phrases.json"
OUT_DOC = ROOT / "translations" / "2corintios.md"
LBF_OUT = HERR / "Biblia-LBF" / "translation" / "nt" / "2corintios.md"
STRONGS = HERR / "MNA" / "datasets" / "rules" / "grc_lemma_strongs.json"

BOOK_CODE = 47
BOOK_LABEL = "2 Corinthians"
DOC_TITLE = "2 Corintios"
...
def load_ble() -> dict[tuple[int, int], str]:
    ...
    for line in BLE.read_text(encoding="utf-8").splitlines():
        m = re.match(r"^(?:2corintios|2Corintios|2\s*Corintios)\s+(\d+):(\d+)\s+(.+)$", line)
        if m:
...
def main() -> int:
    ...
    OUT_MAP.write_text(
        json.dumps({"book": "2corinthians", "version": 1, "phrases": phrase_map}, ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
    )
